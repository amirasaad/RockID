...
| `Production Index Decision` | `Done` | Sprint 42 rejected the Basalt support production-index candidate after expanded-eval rollback evidence and merged with a green release gate. |
| `Same-Label Confidence Shadow` | `Done` | Sprint 43 characterized same-label support as safe to evaluate for production policy. |
| `Same-Label Confidence Policy` | `Done` | Sprint 44 promoted same-label-aware margin while keeping thresholds, index data, and result contracts unchanged; detection and release gates passed. |
| `Basalt Support Index Promotion` | `Done` | Sprint 45 promoted the first Basalt support point after same-label-aware confidence kept detection and release gates green. |
| `Basalt Boundary Readout` | `Done` | Sprint 46 measured the S45 support point against the six-sample dark-boundary pack without changing production behavior. |
| `Asphalt Boundary Readout` | `Done` | Sprint 47 added one ambiguous wet-asphalt pressure case near Basalt while preserving non-rock Top-1 behavior and passed release-build gates. |
| `Coal Boundary Readout` | `Done` | Sprint 48 added one ambiguous dull-coal pressure case near Basalt while preserving non-rock Top-1 behavior. |
| `Dark Boundary Readout Summary` | `Done` | Sprint 49 closed the synthetic dark-boundary readout arc with green release-build evidence and pointed Sprint 50 toward real known-answer evidence. |
| `Real Known-Answer Dark-Confuser Evidence` | `In Progress` | Sprint 50 captures one real, known-answer dark-confuser candidate before any analyzer, index, threshold, UI, API, or result-contract change. |
