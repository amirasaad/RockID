...
| `S36-2` | As a tester, high-priority blockers are addressed quickly. | `beta-priority-high` issues are fixed in-wave or explicitly deferred with a reason. | Must | Pending |
...
| `S36-4` | As the team, retest guidance is clear. | Weekly changelog issue lists fixed items, investigating items, and tester retest asks. | Should | Pending |
